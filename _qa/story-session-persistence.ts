...
const directory = await mkdtemp(join(tmpdir(), 'mist-harbor-session-process-'))
...
try {
  const first = await start(); const second = await start(); const initial = createInitialSave(mistHarborLastLight); const opening = initial.choices[0]!.label
  ...
  const contractOptions = { cartridge: mistHarborLastLight, databasePath, actorTokens: {}, generator: { async send(): Promise<never> { throw new Error('unused') } } }
  assert.throws(() => createStorySessionLab({ ...contractOptions, rulesetVersion: 2 }), /DATABASE_CONTRACT_MISMATCH/)
  assert.throws(() => createStorySessionLab({ ...contractOptions, cartridge: { ...mistHarborLastLight, id: 'different-game' } }), /DATABASE_CONTRACT_MISMATCH/)
