...
const service = createStorySessionLab({
  ...
  sessionNow: () => now++,
  generator: { async send() { return { content: '你把值守卡和控制台的熔断痕迹放在一起核对。\n[state: value="确认应急顺序与故障位置"]\n[choices: "请林芮核对熔断位置"|"比较值守卡与控制台"]' } } },
})
...
try {
  ...
  const privateSave = createInitialSave(mistHarborLastLight); privateSave.objective = 'PRIVATE_OWNER_A_SENTINEL'
  const first = await a.enroll(privateSave, 'owner-a-first')
  const second = await a.enroll(createInitialSave(mistHarborLastLight), 'owner-a-second')
  const foreign = await b.enroll(createInitialSave(mistHarborLastLight), 'owner-b-only')
  ...
  const opened = await journal.open(createInitialSave(mistHarborLastLight))
  ...
  const restarted = await journal.restart(createInitialSave(mistHarborLastLight))
  ...
  enrollmentStore.raw = JSON.stringify({ schema: 1, scope, enrollment: { id: 'new-session', initialSave: createInitialSave(mistHarborLastLight) } })
