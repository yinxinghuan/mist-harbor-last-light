...
const directory = await mkdtemp(join(tmpdir(), 'mist-harbor-story-session-'))
...
try {
  ...
  const store = new Store(); const scope = 'mist-harbor-last-light:deployment:owner-a:zh'
  ...
  const foreign = await client('token-b').enroll(createInitialSave(mistHarborLastLight), 'owner-b-enrollment')
  ...
  const rollbackSession = await client().enroll(createInitialSave(mistHarborLastLight), 'rollback-enrollment')
