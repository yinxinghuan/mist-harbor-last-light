...
const gameBase = '/8a51d15e-7c00-4e27-bc63-25624ee75ac1'
...
for (const locale of ['zh','en'] as const) {
  const service = createStorySessionLab({ cartridge: resolveCartridge(null, locale), databasePath: resolve(directory, `${locale}.sqlite`), actorTokens: { 'qa-ui-a':'qa-ui-a','qa-ui-b':'qa-ui-b','qa-platform-user-a':'qa-platform-user-a','qa-platform-user-b':'qa-platform-user-b' }, generator: { async send(_action, context) { modelCalls += 1; return { content: locale === 'zh' ? `你把值守卡移到黄铜灯下，三道应急顺序和控制台的熔断痕迹正好对应。\n[state: value="核对信号站的应急顺序"]\n[scene_location: location="${context.save.sceneLocation ?? context.save.location}"]\n[choices: "请林芮核对熔断位置"|"比较值守卡与控制台"]` : `You move the duty card beneath the brass lamp. Its three emergency steps align with the burn marks on the console.\n[state: value="Verify the signal station emergency sequence"]\n[scene_location: location="${context.save.sceneLocation ?? context.save.location}"]\n[choices: "Ask Lin Rui to verify the burned contact"|"Compare the duty card with the console"]` } } } })
  services.set(locale, service); bases.set(locale, (await service.listen()).baseUrl)
}
const vite = await createServer({ configFile: resolve('vite.config.ts'), server: { host:'127.0.0.1', port:Number(process.env.STORY_LAB_UI_PORT ?? 5189), strictPort:true }, plugins:[{ name:'mist-harbor-story-session-qa-only', configureServer(server) { server.middlewares.use(async (request,response,next) => {
  ...
  try {
    ...
    const locale=request.headers['x-story-lab-locale']==='en'?'en':'zh'; const platformUser=String(request.headers['x-alteru-user-id']??''); const actor=platformUser==='qa-platform-user-a'||platformUser==='qa-platform-user-b'?platformUser:request.headers['x-story-lab-actor']==='qa-b'?'qa-ui-b':'qa-ui-a'; const route=(request.url ?? '').slice(gameBase.length)
