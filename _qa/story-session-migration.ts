...
import { createStorySessionLab, MIST_HARBOR_SAVE_REPAIR_MIGRATION_ID } from '../server/storySessionLab'
import { StorySessionClient, createStorySessionHttpTransport } from '../src/story/session/storySessionClient'

const directory = await mkdtemp(join(tmpdir(), 'mist-harbor-migration-'))
...
const body = (version: number, ruleset = 1, id = MIST_HARBOR_SAVE_REPAIR_MIGRATION_ID) => JSON.stringify({ migration_id: id, expected_version: version, ruleset_version: ruleset })
...
try {
  const seed = createStorySessionLab({ cartridge: mistHarborLastLight, databasePath, actorTokens: { 'token-a': 'owner-a', 'token-b': 'owner-b' }, generator: { async send(): Promise<never> { throw new Error('unused') } } })
  const seedUrl = (await seed.listen()).baseUrl; const enrolled = await clientFor(seedUrl).enroll(createInitialSave(mistHarborLastLight), 'migration-seed'); await seed.close()
  ...
  const make = () => createStorySessionLab({ cartridge: mistHarborLastLight, databasePath, actorTokens: { 'token-a': 'owner-a', 'token-b': 'owner-b' }, beforeMigrationCommit: async () => { waiting += 1; if (waiting === 2) release(); await barrier }, generator: { async send() { return { content: '值守卡和控制台的故障记录被重新核对。\n[choices: "保存故障记录"|"询问林芮"]' } } } })
  ...
  const extra = await fetch(`${oneUrl}/api/story/sessions/${enrolled.session_id}/migrations`, { method: 'POST', headers: headers(), body: JSON.stringify({ migration_id: MIST_HARBOR_SAVE_REPAIR_MIGRATION_ID, expected_version: 0, ruleset_version: 1, snapshot: {} }) })
  assert.equal(extra.status, 400); assert.equal(auditCount(), 0); assert.equal(readHead(enrolled.session_id), before)

  const results = await Promise.all([c1.migrate(enrolled.session_id, MIST_HARBOR_SAVE_REPAIR_MIGRATION_ID, 0), c2.migrate(enrolled.session_id, MIST_HARBOR_SAVE_REPAIR_MIGRATION_ID, 0)])
  ...
  const reopened = createStorySessionLab({ cartridge: mistHarborLastLight, databasePath, actorTokens: { 'token-a': 'owner-a' }, generator: { async send(): Promise<never> { throw new Error('unused') } } })
  const reopenedUrl = (await reopened.listen()).baseUrl; const replay = await clientFor(reopenedUrl).migrate(enrolled.session_id, MIST_HARBOR_SAVE_REPAIR_MIGRATION_ID, 0)
  ...
  const rollback = createStorySessionLab({ cartridge: mistHarborLastLight, databasePath: rollbackPath, actorTokens: { rollback: 'owner' }, failMigrationBeforeCommit: [MIST_HARBOR_SAVE_REPAIR_MIGRATION_ID], generator: { async send(): Promise<never> { throw new Error('unused') } } })
  const rollbackUrl = (await rollback.listen()).baseUrl; const rollbackClient = clientFor(rollbackUrl, 'rollback'); const rollbackSession = await rollbackClient.enroll(createInitialSave(mistHarborLastLight), 'rollback')
  await assert.rejects(rollbackClient.migrate(rollbackSession.session_id, MIST_HARBOR_SAVE_REPAIR_MIGRATION_ID, 0), { code: 'INTERNAL_ERROR' })
  assert.equal((await rollbackClient.read(rollbackSession.session_id)).version, 0)
  assert.equal((await rollbackClient.migrate(rollbackSession.session_id, MIST_HARBOR_SAVE_REPAIR_MIGRATION_ID, 0)).version, 1)
