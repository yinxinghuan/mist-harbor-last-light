...
import type { ServerDomainResolver } from '../src/story/engine/prologRuleClient'
...
export const MIST_HARBOR_SAVE_REPAIR_MIGRATION_ID = 'mist-harbor-save-v10-repair-2026-09-04'
...
export function createStorySessionLab(options: {
  ...
  domainResolver?: ServerDomainResolver
}) {
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      if (migrationMatch) {
        ...
        if (Object.keys(body).some(key => !['migration_id','expected_version','ruleset_version'].includes(key)) || body.migration_id !== MIST_HARBOR_SAVE_REPAIR_MIGRATION_ID || !counter(body.expected_version) || !counter(body.ruleset_version)) throw new RequestError(400, 'INVALID_MIGRATION')
      ...
      let executed
      try { executed = await executeStoryTurn({ save: current.snapshot, cartridge: options.cartridge, action, generator: options.generator, requestId: String(body.action_id), domainResolver: options.domainResolver }) }
      catch (cause) {
        const code = cause instanceof Error ? cause.message : ''
        send(response, 503, { code: ['RULE_SERVICE_UNAVAILABLE', 'RULE_SERVICE_MISMATCH', 'RULE_SERVICE_CONFIGURATION_REQUIRED'].includes(code) ? code : 'MODEL_UNAVAILABLE' })
        return
      }
