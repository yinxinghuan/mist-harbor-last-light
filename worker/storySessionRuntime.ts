...
interface RuntimeOptions {
  ...
  executeTurn(options: { save: StorySave; cartridge: any; action: string; locale: Locale; generator: any; requestId?: string; domainResolver?: any }): Promise<{ save: StorySave; source: string }>
  generator: any
  createDomainResolver?: (env: any, context: { sessionId: string; actorId: string }) => any
...
export function createStorySessionRuntime(options: RuntimeOptions) {
  class StorySessionAuthority {
    ...
    async fetch(request: Request) {
      try {
        ...
        try { executed = await options.executeTurn({ save: structuredClone(current.snapshot), cartridge, action, locale: current.snapshot.locale, generator: options.generator, requestId: body.action_id, domainResolver: options.createDomainResolver?.(this.env, { sessionId, actorId: owner }) }) }
        catch (cause) {
  ...
  async function handleStoryApi(request: Request, env: any) {
    ...
    const owner = await digest(hasPlatformIdentity
      ? { mode: 'alteru-user-id-v1-experimental', game: options.gameId, user: platformUserId }
      : { mode: 'anonymous-capability-v1', game: options.gameId, token })
