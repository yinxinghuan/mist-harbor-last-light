...
import type { ServerDomainResolver } from './prologRuleClient'
...
export async function executeStoryTurn(options: {
  ...
  const domainResolution = options.domainResolver
    ? await options.domainResolver({ save: base, cartridge, action, requestId: options.requestId ?? `local-${base.scene}` })
    : resolveDomainAction(base, cartridge, action)
